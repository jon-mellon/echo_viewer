// Frozen display rules for compatibility tests.
function visNodeData(group, layoutPoint, layoutParams = {}) {
  const isIv = group.type === "iv";
  const isDv = group.type === "dv";
  const diagnosticCandidateIds = state.showCollidersOnly
    ? state.colliderGroupIds
    : state.confounderGroupIds;
  const diagnosticPathIds = state.showCollidersOnly
    ? state.colliderPathGroupIds
    : state.confounderPathGroupIds;
  const diagnosticRole = confounderPathNodeRole({
    confounderIds: diagnosticCandidateIds,
    pathIds: diagnosticPathIds,
  }, group.group_id, {
    ivId: state.project?.iv_group_id,
    dvId: state.project?.dv_group_id,
  });
  const isConfounder = state.showConfoundersOnly
    && diagnosticRole === "confounder"
    && !isIv
    && !isDv;
  const isCollider = state.showCollidersOnly
    && diagnosticRole === "confounder"
    && !isIv
    && !isDv;
  const isPathMediator = (state.showConfoundersOnly || state.showCollidersOnly)
    && diagnosticRole === "mediator";
  const isSelected = group.group_id === state.activeGroupId;
  const color = isConfounder ? "#c62828" : isCollider ? "#7c3aed" : isPathMediator ? "#6b7280" : groupColor(group);
  const roleTag = isIv ? " (IV)" : isDv ? " (DV)" : "";
  const mediatorTag = isPathMediator ? " · path mediator" : "";
  const maxChars = layoutParams.nodeMaxWidth <= 170 ? 18 : 22;
  return {
    id: group.group_id,
    label: wrapDagLabel(group.label, maxChars),
    title: isConfounder || isCollider
      ? undefined
      : `${escapeHtml(group.label)}${roleTag} · ${group.variable_ids.length} variables${mediatorTag}`,
    x: layoutPoint?.x || 0,
    y: layoutPoint?.y || 0,
    fixed: { x: true, y: true },
    mass: 1 + Math.min(3, Math.sqrt(group.variable_ids.length || 1) / 6),
    color: {
      background: color,
      border: isSelected ? "#b83b5e" : isConfounder ? "#7f1d1d" : isCollider ? "#4c1d95" : isPathMediator ? "#374151" : "#fbfcfa",
      highlight: { background: color, border: isConfounder ? "#4c0d0d" : isCollider ? "#2e1065" : isPathMediator ? "#1f2937" : "#b83b5e" },
      hover: { background: color, border: "#ffffff" },
    },
    shape: (isIv || isDv) ? "ellipse" : "box",
    borderWidth: isSelected ? 3 : 2.4,
    // Light outline keeps dense links visually separated from node bodies.
    shadow: isSelected
      ? { enabled: true, color: "rgba(184,59,94,0.45)", size: 12, x: 0, y: 0 }
      : isConfounder
        ? { enabled: true, color: "rgba(198,40,40,0.34)", size: 11, x: 0, y: 2 }
        : isCollider
          ? { enabled: true, color: "rgba(124,58,237,0.34)", size: 11, x: 0, y: 2 }
        : isPathMediator
          ? { enabled: true, color: "rgba(55,65,81,0.28)", size: 9, x: 0, y: 2 }
      : { enabled: true, color: "rgba(25, 38, 34, 0.14)", size: 7, x: 0, y: 2 },
  };
}

function edgeVisualData(link) {
  const isSelected = link.edge_id === state.selectedEdgeId;
  const isTarget = link.is_target_relation;
  return {
    color: {
      color: isSelected ? "#b83b5e" : isTarget ? "#1e4fa0" : "#7ba096",
      highlight: "#b83b5e",
      hover: "#805214",
      inherit: false,
      opacity: isSelected ? 0.95 : isTarget ? 0.74 : 0.38,
    },
    dashes: link.is_manual ? [6, 4] : false,
    width: isSelected ? 3.6 : isTarget ? 2.6 : 1.0,
  };
}

function edgeHoverText(link) {
  const { sourceLabel, targetLabel, arrow } = directedGroupLabels(link, "<->");
  const lines = [`${sourceLabel} ${arrow} ${targetLabel}`];
  if (link.is_target_relation) lines.push("Target relation");
  if (link.is_manual) lines.push("Manual edge");
  const keys = [...new Set([...link.a_to_b_paper_table_keys, ...link.b_to_a_paper_table_keys])].slice(0, 8);
  if (keys.length) { lines.push("Papers/tables:"); lines.push(...keys); }
  else if (!link.is_manual) lines.push("No provenance.");
  return lines.join("\n");
}

function escapeHtml(value) {
  return String(value ?? "")
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
    .replaceAll('"', "&quot;");
}
